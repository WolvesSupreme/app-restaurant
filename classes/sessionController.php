    ...
    function validateSession()
    {
        ...
        if ($this->existsSession()) {
            ...
            if ($this->isPublic()) {
                ...
                $this->redirectDefaultSiteByRoleAdmin($role);
    ...
    private function redirectDefaultSiteByRole($role)
    {
        ...
        for ($i = 0; $i < sizeof($this->sites); $i++) {
            if ($this->sites[$i]['role'] === $role) {
                $url = '/restaurant-app/' . $this->sites[$i]['site'];
                break;
            }
        }
        header('location: ' . $url);
    }
    private function redirectDefaultSiteByRoleAdmin($role)
    {
        $url = '';
        for ($i = 0; $i < sizeof($this->sites); $i++) {
            if ($this->sites[$i]['role'] === $role) {
                $url = '/restaurant-app/admin' . $this->sites[$i]['site'];
                break;
            }
        }
        header('location: ' . $url);
